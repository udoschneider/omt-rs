...
mod tests {
    ...
    /// Create simple test YUY2 data for gray image
    fn create_gray_yuy2_data(width: usize, height: usize, yuv_range: YuvRange) -> Vec<u8> {
        // For YUY2, each 2 pixels requires 4 bytes: Y0, U0, Y1, V0
    ...
    /// Create simple test YUY2 data for color bars
    fn create_color_bars_yuy2_data(width: usize, height: usize, yuv_range: YuvRange) -> Vec<u8> {
        // For YUY2, each 2 pixels requires 4 bytes: Y0, U0, Y1, V0
        let data_size = width * height * 2;
        let mut yuy2_data = vec![0u8; data_size];
        ...
        // Fill YUY2 data with color bars
        for y in 0..height {
            for x in (0..width).step_by(2) {
                ...
                yuy2_data[base_idx + 1] = u_value; // U
                yuy2_data[base_idx + 2] = y_value1; // Y1
                yuy2_data[base_idx + 3] = v_value; // V
            }
        }
    ...
    #[test]
    fn test_yuy2_to_rgb8_bt601_limited() {
        let width = 8;
        let height = 8;
        let stride = width * 2; // YUY2 uses 2 bytes per pixel

        // Create test data for gray image
        let yuy2_data = create_gray_yuy2_data(width, height, Limited);

        // Convert YUY2 to RGB8
        let actual_rgb_result = yuy2_to_rgb8(&yuy2_data, width, height, stride, Limited, Bt601);
        assert!(
            actual_rgb_result.is_some(),
            "yuy2_to_rgb8 should return Some for BT601 Limited range"
        );
    ...
    #[test]
    fn test_yuy2_to_rgb8_bt601_full() {
        let width = 8;
        let height = 8;
        let stride = width * 2; // YUY2 uses 2 bytes per pixel

        // Create test data for gray image
        let yuy2_data = create_gray_yuy2_data(width, height, Full);

        // Convert YUY2 to RGB8
        let actual_rgb_result = yuy2_to_rgb8(&yuy2_data, width, height, stride, Full, Bt601);
        assert!(
            actual_rgb_result.is_some(),
            "yuy2_to_rgb8 should return Some for BT601 Full range"
        );
    ...
    #[test]
    fn test_yuy2_to_rgb8_bt709_limited() {
        let width = 8;
        let height = 8;
        let stride = width * 2; // YUY2 uses 2 bytes per pixel
        ...
        // Convert YUY2 to RGB8
        let actual_rgb_result = yuy2_to_rgb8(&yuy2_data, width, height, stride, Limited, Bt709);
        assert!(
            actual_rgb_result.is_some(),
            "yuy2_to_rgb8 should return Some for BT709 Limited range"
        );
    ...
    #[test]
    fn test_yuy2_to_rgb8_bt709_full() {
        let width = 8;
        let height = 8;
        let stride = width * 2; // YUY2 uses 2 bytes per pixel
        ...
        // Convert YUY2 to RGB8
        let actual_rgb_result = yuy2_to_rgb8(&yuy2_data, width, height, stride, Full, Bt709);
        assert!(
            actual_rgb_result.is_some(),
            "yuy2_to_rgb8 should return Some for BT709 Full range"
        );
    ...
    #[test]
    fn test_yuy2_to_rgba8_bt601_limited() {
        let width = 8;
        let height = 8;
        let stride = width * 2; // YUY2 uses 2 bytes per pixel
        ...
        // Convert YUY2 to RGBA8
        let actual_rgba_result = yuy2_to_rgba8(&yuy2_data, width, height, stride, Limited, Bt601);
        assert!(
            actual_rgba_result.is_some(),
            "yuy2_to_rgba8 should return Some for BT601 Limited range"
        );
    ...
    #[test]
    fn test_yuy2_to_rgba8_bt601_full() {
        let width = 8;
        let height = 8;
        let stride = width * 2; // YUY2 uses 2 bytes per pixel
        ...
        // Convert YUY2 to RGBA8
        let actual_rgba_result = yuy2_to_rgba8(&yuy2_data, width, height, stride, Full, Bt601);
        assert!(
            actual_rgba_result.is_some(),
            "yuy2_to_rgba8 should return Some for BT601 Full range"
        );
    ...
    #[test]
    fn test_yuy2_to_rgba8_bt709_limited() {
        let width = 8;
        let height = 8;
        let stride = width * 2; // YUY2 uses 2 bytes per pixel
        ...
        // Convert YUY2 to RGBA8
        let actual_rgba_result = yuy2_to_rgba8(&yuy2_data, width, height, stride, Limited, Bt709);
        assert!(
            actual_rgba_result.is_some(),
            "yuy2_to_rgba8 should return Some for BT709 Limited range"
        );
    ...
    #[test]
    fn test_yuy2_to_rgba8_bt709_full() {
        let width = 8;
        let height = 8;
        let stride = width * 2; // YUY2 uses 2 bytes per pixel
        ...
        // Convert YUY2 to RGBA8
        let actual_rgba_result = yuy2_to_rgba8(&yuy2_data, width, height, stride, Full, Bt709);
        assert!(
            actual_rgba_result.is_some(),
            "yuy2_to_rgba8 should return Some for BT709 Full range"
        );
    ...
    #[test]
    fn test_yuy2_conversion_with_color_bars() {
        let width = 16;
        let height = 8;
        let stride = width * 2; // YUY2 uses 2 bytes per pixel

        // Test with BT601 Limited range
        let yuy2_data = create_color_bars_yuy2_data(width, height, Limited);

        // Convert YUY2 to RGB8
        let rgb_result = yuy2_to_rgb8(&yuy2_data, width, height, stride, Limited, Bt601);
        assert!(
            rgb_result.is_some(),
            "yuy2_to_rgb8 should return Some for color bars"
        );

        // Convert YUY2 to RGBA8
        let rgba_result = yuy2_to_rgba8(&yuy2_data, width, height, stride, Limited, Bt601);
        assert!(
            rgba_result.is_some(),
            "yuy2_to_rgba8 should return Some for color bars"
        );
        ...
        let rgb_result_bt709 = yuy2_to_rgb8(&yuy2_data, width, height, stride, Limited, Bt709);
        let rgba_result_bt709 = yuy2_to_rgba8(&yuy2_data, width, height, stride, Limited, Bt709);

        assert!(
            rgb_result_bt709.is_some(),
            "yuy2_to_rgb8 should return Some for BT709"
        );
        assert!(
            rgba_result_bt709.is_some(),
            "yuy2_to_rgba8 should return Some for BT709"
        );
    ...
    fn test_yuy2_conversion_edge_cases() {
        ...
        for (width, height) in test_dimensions {
            ...
            assert!(
                rgb_result.is_some(),
                "yuy2_to_rgb8 should return Some for {}x{}",
                width,
                height
            );
            ...
            assert!(
                rgba_result.is_some(),
                "yuy2_to_rgba8 should return Some for {}x{}",
                width,
                height
            );
